...
import { ShoppingCart, Eye, Heart, Zap, Phone } from 'lucide-react';
...
export const ProductCard: React.FC<{ product: Product }> = ({ product }) => {
    ...
    // Engine Injection: Real-time Stock Check
        ...
        <div className="group bg-white rounded-xl shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden border border-stone-100 flex flex-col h-full relative">
            <div className="relative aspect-square overflow-hidden cursor-pointer" onClick={() => navigate(`/product/${product.id}`)}>
                ...
                <button
                    ...
                    className={`absolute top-3 right-3 p-2 rounded-full shadow-md transition-all z-10 ${isWishlisted ? 'bg-white text-red-500' : 'bg-white/80 text-stone-400 hover:text-red-500'}`}
                >
            ...
            <div className="p-5 flex flex-col flex-grow">
                ...
                <h3 className="text-lg font-bold text-stone-800 mb-2 cursor-pointer hover:text-amber-700 transition">{product.name}</h3>
                ...
                <div className="flex items-center justify-between mt-auto pt-4 gap-2">
                    ...
                    <button
                        onClick={(e) => { e.stopPropagation(); if (stock > 0) addToCart(product); }}
                        disabled={stock === 0}
                        className={`flex items-center gap-2 text-white px-4 py-2 rounded-full transition shadow-lg ${stock > 0 ? 'bg-stone-900 hover:bg-amber-600' : 'bg-stone-400 cursor-not-allowed'}`}
                    >
